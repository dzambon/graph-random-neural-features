...
# Build model
model_ecc = Model(inputs=[X_in, A_in, E_in, I_in], outputs=output_ecc, name="ECC")
opt = Adam(lr=learning_rate)
loss_fn = MeanSquaredError()

# GRNF
psi = GraphRandomNeuralFeatures(256, activation="relu")([X_in, A_in, E_in, I_in])
d1 = Dense(16, activation="relu")(psi)
output_grnf = Dense(n_out)(d1)

# Build model
model_grnf = Model(inputs=[X_in, A_in, E_in, I_in], outputs=output_grnf, name="GRNF")

################################################################################
# FIT MODELS
################################################################################

for model in [model_ecc, model_grnf]:
    @tf.function(
